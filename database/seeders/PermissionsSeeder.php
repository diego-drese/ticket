...
class PermissionsSeeder extends Seeder
{
    ...
    private array $extraPermissions = [
        ...
        'List timesheet data', 'View timesheet dashboard', 'List road map'
    ];

    private string $defaultRole = 'Administrador';
    private string $operatorRole = 'Operador';
    ...
    public function run(){
        ...
        if(!Role::where('name', $this->defaultRole)->exists()){
            // Create default role
        ...
        if(!Role::where('name', $this->operatorRole)->exists()){
            // Create default role
            $role = Role::firstOrCreate([
                'name' => $this->operatorRole
            ]);
            $settings = app(GeneralSettings::class);
            $settings->default_role = $role->id;
            $settings->save();
            ...
            // Add all permissions to default role
            $tickets = Permission::whereIn('name', ['List tickets', 'Update ticket', 'View ticket'])->get()->pluck('name')->toArray();
            $role->syncPermissions($tickets);
